...
if sys.stderr.isatty():
    import colored
    to_colorizer, colored_err_p = Pipe(duplex=False)
    colorizer = Process(target=colored.main, args=(to_colorizer, sys.stderr))
    colorizer.daemon = True
    colorizer.start()
    colored_err = os.fdopen(colored_err_p.fileno(), 'w', 0)
else:
    colored_err = sys.stderr
...
__all__ = ['setup', 'run', 'autoclean', 'main', 'shell', 'fabricate_version',
           ...
           'SmartRunner', 'Builder', 'printerr', 'proc', 'script_name', 'docopt', 'shlex']
...
def _shell(args, input=None, silent=True, shell=False, ignore_status=False, **kwargs):
    ...
    try:
        proc = subprocess.Popen(command, stdin=stdin, stdout=stdout,
                                stderr=colored_err, shell=shell, **kwargs)
    except OSError, e:
...
def setup(builder=None, default=None, proc_vars={}, **kwargs):
    ...
    global _setup_builder, _setup_default, _setup_kwargs, _proc_vars
    _setup_builder = builder
    _setup_default = default
    _proc_vars = proc_vars
...
def _set_default_builder():
    ...
    if default_builder is None:
        default_builder = Builder(**_setup_kwargs)
...
def main(globals_dict=None, build_dir=None, extra_options=None, builder=None,
    ...
    if options.dir:
        if 'dirs' not in kwargs:
            kwargs['dirs'] = []
        kwargs['dirs'].extend(options.dir)
    if options.keep:
...
def proc(args):
    ...
    if len(args) == 1:
        args = c.vformat(args[0], [], _proc_vars)
        args = shlex.split(c.vformat(args, [], _proc_vars))
    if _proc_vars['--dry-run']:
